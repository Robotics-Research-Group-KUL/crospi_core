...
#include "robot_driver_manager.hpp"
// #include "crospi_node_utils/robot_driver_manager.hpp"


namespace etasl {

    RobotDriverManager::RobotDriverManager(rclcpp_lifecycle::LifecycleNode::SharedPtr node,
    ...
    RobotDriverManager::~RobotDriverManager() {
    ...
    void RobotDriverManager::construct_driver(int num_joints){
        ...
        if (simulation_){
          driver_loader_ = std::make_shared<pluginlib::ClassLoader<etasl::RobotDriver>>("crospi_core", "etasl::RobotSimulator"); //This works because etasl::RobotSimulator inherits from etasl::RobotDriver
          robotplugintype = "robotsimulator";
        }
        else{
          driver_loader_ = std::make_shared<pluginlib::ClassLoader<etasl::RobotDriver>>("crospi_core", "etasl::RobotDriver");
    ...
    void RobotDriverManager::on_configure(Context::Ptr ctx){
            ...
            if(value){
              ...
              if (key == "joint_vel") {
                for (unsigned int i = 0; i < input_channels_feedback.joint_vel.size(); ++i) {
                  input_channels_feedback.joint_vel[i] = ctx->getInputChannel<double>(jsonchecker_->asString(param_robot, "robotdriver/name_expr_" + key) + "_" + std::to_string(i));
                }
              } 
              else if (key == "joint_torque") {
                for (unsigned int i = 0; i < input_channels_feedback.joint_torque.size(); ++i) {
                  input_channels_feedback.joint_torque[i] = ctx->getInputChannel<double>(jsonchecker_->asString(param_robot, "robotdriver/name_expr_" + key) + "_" + std::to_string(i));
                }
              }
              else if (key == "joint_current") {
                for (unsigned int i = 0; i < input_channels_feedback.joint_current.size(); ++i) {
                  input_channels_feedback.joint_current[i] = ctx->getInputChannel<double>(jsonchecker_->asString(param_robot, "robotdriver/name_expr_" + key) + "_" + std::to_string(i));
                }
              }
    ...
    std::vector<double> RobotDriverManager::get_position_feedback(){
    ...
    void RobotDriverManager::on_activate(){

    }


    void RobotDriverManager::on_deactivate(){

    }


    void RobotDriverManager::on_cleanup(){

    }

    void RobotDriverManager::finalize(){
    ...
    std::shared_ptr<t_manager::thread_t> RobotDriverManager::create_thread_str(std::atomic<bool> & stopFlag){
