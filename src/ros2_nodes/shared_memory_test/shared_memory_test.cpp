#include "shared_memory_test.hpp"
...
ConsumerNode::ConsumerNode(): Node("shared_memory_test")
, time(0.0)
{
  ...
  this->get_node_base_interface()->get_context()->add_pre_shutdown_callback(std::bind( &ConsumerNode::safe_shutdown, this)); // Adds safe_shutdown as callback before shutting down, e.g. with ctrl+c. This methods returns rclcpp::OnShutdownCallbackHandle shutdown_cb_handle

  

}

void ConsumerNode::safe_shutdown(){
...
void ConsumerNode::construct(FeedbackMsg* fb, SetpointMsg* sp, double periodicity_val, triple_buffer_port_t *port_setpoints_p) {
    // feedback_ptr = fb;
    // setpoint_ptr = sp; 
    port_setpoints = port_setpoints_p;
    ...
    timer_ = this->create_wall_timer(std::chrono::milliseconds(periodicity_ms), std::bind(&ConsumerNode::update, this));
...
void ConsumerNode::consume_data() {
  ...
  ret = acquire_from_triple_buffer_port(port_setpoints);
    ...
    for (unsigned int i=0; i<DOF; ++i) {
      savepoint = setpoint_ptr->load1.data[i];
      savepoint = setpoint_ptr->load2.data[i];
      savepoint = setpoint_ptr->load3.data[i];
      savepoint = setpoint_ptr->load4.data[i];
      savepoint = setpoint_ptr->load5.data[i];
      savepoint = setpoint_ptr->load6.data[i];
      savepoint = setpoint_ptr->load7.data[i];
      savepoint = setpoint_ptr->load8.data[i];
      savepoint = setpoint_ptr->load9.data[i];
      savepoint = setpoint_ptr->load10.data[i];
    }
    ...
    long long latency_ns = std::chrono::duration_cast<std::chrono::nanoseconds>(now - setpoint_ptr->timestamp).count();
  ...
  ret = release_from_triple_buffer_port(port_setpoints);
...
Producer::Producer(FeedbackMsg* fb, SetpointMsg* sp, double periodicity_val, triple_buffer_port_t *port_setpoints_p):
 periodicity(periodicity_val) //Expressed in seconds
, time(0.0)
...
, frequency(0.5) // Frequency of the sine wave
{
  ...
  SetpointMsg initialized_setpoint{}; // Assuming DOF is the required constructor argument

      
    unsigned char i;
    char* buffer_data_ptr;
    SetpointMsg* chunk_data_ptr;
    triple_buffer_t *buffer;

    buffer = get_buffer_from_port(port_setpoints);
    if (buffer != nullptr) {
    ...
    // Initialize the buffer data to zero
    for (i = 0; i < 3; ++i){
      // buffer->data[i * n_bytes_per_chunk + j] = 0; 
      // n_bytes_per_chunk =  sizeof(*setpoint_shared_ptr);

      buffer_data_ptr = get_data_chunk_from_buffer(buffer);
      chunk_data_ptr = (SetpointMsg *)(&buffer_data_ptr[i* sizeof(initialized_setpoint)]);
      // chunk_data_ptr = (SetpointMsg *)(buffer_data_ptr + i * sizeof(initialized_setpoint)); //Equivalent to above

      if (chunk_data_ptr != nullptr) {
        *chunk_data_ptr = initialized_setpoint; // Use the assignment operator to copy values
      } else {
          std::cerr << "Error: chunk_data_ptr is null!" << std::endl;
      }
    } 
  ...
  // ret = acquire_from_triple_buffer_port(port_setpoints);
	// if (TRIPLE_BUFFER_ACQUIRE_SUCCEEDED == ret) {
  //   setpoint_ptr = (SetpointMsg *)get_data_chunk_from_port(port_setpoints);
  //   if (setpoint_ptr != nullptr) {
  //       *setpoint_ptr = initialized_setpoint; // Use the assignment operator to copy values
  //   } else {
  //       std::cerr << "Error: setpoint_ptr is null!" << std::endl;
  //   }
  // }
  // else{
...
void Producer::produce_data(){
  ...
  ret = acquire_from_triple_buffer_port(port_setpoints);
    ...
    setpoint_ptr = (SetpointMsg *)get_data_chunk_from_port(port_setpoints);
    setpoint_ptr->velocity.fs = etasl::NewData;
    // std::cout << "size: " << setpoint_ptr->velocity.data.size() << std::endl;
    for (unsigned int i=0; i<DOF; ++i) {
      setpoint_ptr->velocity.data[i] = joint_velocities[i];
    }

    for (unsigned int i=0; i<DOF; ++i) {
      setpoint_ptr->load1.data[i] = joint_velocities[i];
      setpoint_ptr->load2.data[i] = joint_velocities[i];
      setpoint_ptr->load3.data[i] = joint_velocities[i];
      setpoint_ptr->load4.data[i] = joint_velocities[i];
      setpoint_ptr->load5.data[i] = joint_velocities[i];
      setpoint_ptr->load6.data[i] = joint_velocities[i];
      setpoint_ptr->load7.data[i] = joint_velocities[i];
      setpoint_ptr->load8.data[i] = joint_velocities[i];
      setpoint_ptr->load9.data[i] = joint_velocities[i];
      setpoint_ptr->load10.data[i] = joint_velocities[i];
    }
  ...
  setpoint_ptr->timestamp = std::chrono::steady_clock::now();
...
int main(int argc, char * argv[])
{
    ...
    std::shared_ptr<etasl::FeedbackMsg> feedback_shared_ptr = std::make_shared<etasl::FeedbackMsg>(DOF);
    std::shared_ptr<etasl::SetpointMsg> setpoint_shared_ptr = std::make_shared<etasl::SetpointMsg>();
    

    // ----------------- Stuff related to triple buffer: START ----------
    char *data;
    triple_buffer_t *buffer;
    
    uint64_t n_chunks_desired;
    uint64_t n_chunks_allocated;
    uint64_t n_bytes_per_chunk;
    bool force_buffer_data_alignment;
    bool force_n_chunks_allocated_geq_pow2;
    triple_buffer_error_code_t ret;
    
    n_chunks_desired = 3;
    n_bytes_per_chunk =  sizeof(*setpoint_shared_ptr.get());
    force_buffer_data_alignment = true;
    force_n_chunks_allocated_geq_pow2 = true;
    data = alloc_chunks_memory(n_chunks_desired, &n_chunks_allocated, n_bytes_per_chunk, force_buffer_data_alignment, force_n_chunks_allocated_geq_pow2);
    

    if (TRIPLE_BUFFER_ALLOCATE_SUCCEEDED != allocate_triple_buffer(&buffer, n_chunks_allocated, data, n_bytes_per_chunk )) {
      std::cout << "impossible to create triple_buffer_t or content" << std::endl;
      free(data);
      exit(1);
    }
    ...
    // // Initialize the buffer data to zero
    // for (i = 0; i < n_chunks_allocated; ++i) 
    //   for (j = 0; j < n_bytes_per_chunk; ++j) 
    //     // buffer->data[i * n_bytes_per_chunk + j] = 0; 
    //     buffer_data_ptr = (SetpointMsg *)get_data_chunk_from_buffer(buffer);
 

    initialize_triple_buffer(buffer);

    // SetpointMsg empty_setpoint(DOF);

    // if (setpoint_ptr != nullptr) {
    //     *setpoint_ptr = empty_setpoint; // Use the assignment operator to copy values
    // } else {
    //     std::cerr << "Error: setpoint_ptr is null!" << std::endl;
    // }

    triple_buffer_port_t *port_setpoints_producer;
    triple_buffer_port_t *port_setpoints_consumer;

    ret = register_to_triple_buffer_port(buffer, BUFFER_ROLE_PRODUCER, &port_setpoints_producer);
    assert(TRIPLE_BUFFER_REGISTER_SUCCEEDED == ret);
  
    ret = register_to_triple_buffer_port(buffer, BUFFER_ROLE_CONSUMER, &port_setpoints_consumer);
    assert(TRIPLE_BUFFER_REGISTER_SUCCEEDED == ret);


    
    // ----------------- Stuff related to triple buffer END----------
    
    
    // Initialize node
    rclcpp::init(argc, argv);
    std::shared_ptr<ConsumerNode> my_consumer_node = std::make_shared<ConsumerNode>();


    my_consumer_node->construct(feedback_shared_ptr.get(), setpoint_shared_ptr.get(), periodicity_consumer, port_setpoints_consumer);
    // Producer my_producer(feedback_shared_ptr.get(), setpoint_shared_ptr.get(), periodicity_producer);
    // rclcpp::executors::StaticSingleThreadedExecutor executor;

    std::shared_ptr<Producer> my_producer_ptr = std::make_shared<Producer>(feedback_shared_ptr.get(), setpoint_shared_ptr.get(), periodicity_producer, port_setpoints_producer);
