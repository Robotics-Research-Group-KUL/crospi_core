...
class ConfigureEtasl(ServiceState):
    def __init__(self, node: Node) -> None:
        super().__init__(
            ...
            ["activate_etasl"],  # outcomes. Includes (SUCCEED, ABORT)
...
class ActivateEtasl(ServiceState):
    def __init__(self, node: Node) -> None:
        super().__init__(
            ...
            "/etasl_node/change_state",  # service name
            self.create_request_handler,  # cb to create the request
            ["outcome4"],  # outcomes. Includes (SUCCEED, ABORT)
            self.response_handler  # cb to process the response
        )
...
class ServiceClientDemoNode(Node):
    ...
    def __init__(self):
        ...
        sm.add_state("ADD_TWO_INTS", AddTwoIntsState(self),
                     transitions={"outcome1": "CONFIG_ETASL",
                                  SUCCEED: "outcome4",
                                  ABORT: "outcome4"})

        sm.add_state("CONFIG_ETASL", ConfigureEtasl(self),
                transitions={"activate_etasl": "ACTIVATE_ETASL",
                            SUCCEED: "outcome4",
                            ABORT: "outcome4"})
        ...
        # sm.add_state("CONFIG_ETASL", CbState([SUCCEED], set_ints),
        #              transitions={SUCCEED: "outcome4"})
