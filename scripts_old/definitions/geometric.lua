...
-- should be in the original library, but for now in a separate file
...
-- translates with a scalar value *a* along the X-axis:
--  - *a* value to translate.
--
-- Returns:
--  - Frame expression.
function translate_x(a)
    local msg =
    "translate_x( a ) with a scalar variable or expression\n   translate along the X-axis with the given value\n\n"
    if a == "help" then
        print(msg)
        return
    end
...
-- translates with a scalar value *a* along the Y-axis:
--  - *a* value to translate.
--
-- Returns:
--  - Frame expression.
...
--- translates with a scalar value *a* along the Z-axis:
---  - *a* value to translate.
---
--- Returns:
---  - Frame expression.
...
-- rotates with a scalar angle *a* along the X-axis:
--  - *a* amgle over which to rotate.
--
-- Returns:
--  - Frame expression.
function rotate_x(a)
...
-- rotates with a scalar angle *a* along the Y-axis:
--  - *a* amgle over which to rotate.
--
-- Returns:
--  - Frame expression.
function rotate_y(a)
    local msg =
    "rotate_y( a ) with a scalar variable or expression\n   rotates along the Y-axis with the given value\n\n"
    if a == "help" then
        print(msg)
        return
    end
...
--- rotates with a scalar angle *a* along the Z-axis:
---  - *a* amgle over which to rotate.
---
--- Returns:
---  - Frame expression.
...
-- angle between directed vectors: 
--  - *a* first vector
--  - *b* second vector
-- 
-- Result :
--  - angle from *a* to *b* with a range of [0..pi[
...
-- smallest angle between two lines indicated by vectors: 
--  - *a* first vector
--  - *b* second vector
-- 
-- Result :
--  - angle from *a* to *b* with a range of [0..pi/2[
...
--  - *P* a plane represented by a frame *P* such that the plane corresponds to the
-- XY-directions of the frame and goes through the origin of the frame.
--  - *a* an expression for a directional vector
--
...
--  - *P1* an oriented plane represented by a frame *P1* such that the plane corresponds to the
-- XY-directions of the frame and goes through the origin of the frame.
--  - *P2* an oriented plane represented by a frame *P2* such that the plane corresponds to the
-- XY-directions of the frame and goes through the origin of the frame.
--
-- Returns:
--  - angle between the two oriented planes.
...
--  - *p1* vector indicating point *p1*
--  - *p2* vector indicating point *p2*
--
-- Returns:
...
--  - *Line* : a frame representing a line.  The direction is indicated by the z-axis of
-- the frame and the line goes through the origin.
--  - *point* : a vector indicating a point.
--
-- Returns:
...
function distance_line_point(Line,point)
    local msg=[[ distance_line_point(Line, point) 
                 INPUT: 
                    Line :  a frame expression representing a line 
...
-- - *L1* : a frame representing a line.  The direction is indicated by the z-axis of
-- the frame and the line goes through the origin.
-- - *L2* : a frame representing a line.  The direction is indicated by the z-axis of
-- the frame and the line goes through the origin.
--
-- Returns:
--  - distance between line *L1* and line *L2*
...
-- returns an expression for the image coordinates using a camera model
-- from Chaumette2006-RAM paper:
--  - *cu*,*cv* : pixel coordinates of the middle in the u- and the v-direction (value)
--  - *alpha* : ratio of pixel dimensions (value)
--  - f : focal length in pixels (value)
--  - *CF* : camera frame with camera along the Z-axis, X-axis horizontal and Y-axis pointing
--  to the top-side of the image. (expression)
--  - *p* : point where *CF* and *p* need to be expressed in the same frame (expression).
--
-- **EXAMPLE**:
-- ```
-- using 36 x 24 mm full frame camera with 50mm lens 
-- at 1920x1280 pixels, with p in [m] :
--   alpha = 1
...
-- the distance between two line segments given by their start- and end-points:
--  - *a1* : start point first segment (expression)
--  - *a2* : end point first segment (expression)
--  - *b1* : start point second segment (expression)
--  - *b2* : end point second segment (expression)
--
-- Returns:
...
-- the distance between a line segments given by its start- and end-point, and a point:
--  - *a1* : start point first segment (expression)
--  - *a2* : end point first segment (expression)
--  - *p* : point.
--
