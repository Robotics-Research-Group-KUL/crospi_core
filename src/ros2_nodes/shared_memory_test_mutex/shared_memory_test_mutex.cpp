...
#include "shared_memory_test_mutex.hpp"
...
ConsumerNode::ConsumerNode(): Node("shared_memory_test_mutex")
, time(0.0)
{
...
void ConsumerNode::construct(FeedbackMsg* fb, SetpointMsg* sp, double periodicity_val) {
    feedback_ptr = fb;
    setpoint_ptr = sp; 
...
void ConsumerNode::update() {
   // The consumer node acts as the ROBOT DRIVER
    ...
    float amplitude = 1.0;
    float frequency = 0.5;
    joint_state_msg.position[4]  = amplitude*sin(2*3.1416*frequency*time) + initial_joints[4];
    joint_state_msg.velocity[4]  = amplitude*2*3.1416*frequency*cos(2*3.1416*frequency*time);

    time += periodicity; // Update the time for the next iteration
    publisher_->publish(joint_state_msg);
}

Producer::Producer(FeedbackMsg* fb, SetpointMsg* sp, double periodicity_val):
 periodicity(periodicity_val) //Expressed in seconds
, time(0.0)
...
int main(int argc, char * argv[])
{
    ...
    std::shared_ptr<etasl::SetpointMsg> setpoint_shared_ptr = std::make_shared<etasl::SetpointMsg>(DOF);
    ...
    my_consumer_node->construct(feedback_shared_ptr.get(), setpoint_shared_ptr.get(), periodicity_consumer);
    // Producer my_producer(feedback_shared_ptr.get(), setpoint_shared_ptr.get(), periodicity_producer);
    // rclcpp::executors::StaticSingleThreadedExecutor executor;

    std::shared_ptr<Producer> my_producer_ptr = std::make_shared<Producer>(feedback_shared_ptr.get(), setpoint_shared_ptr.get(), periodicity_producer);
